...
function snap_target(tag)
	...
	if target then
		...
		setProperty("camFollow.x", cur.x)
		setProperty("camFollow.y", cur.y)
...
function onCreatePost()
	...
	if runHaxeCode([[return game.gf != null;]]) and not targets.gf then
		set_target("gf",
...
function onUpdatePost(elapsed)
	if not freeze then
		...
		setProperty("camFollow.x", cur.x + shaking.g.x)
		setProperty("camFollow.y", cur.y + shaking.g.y)
