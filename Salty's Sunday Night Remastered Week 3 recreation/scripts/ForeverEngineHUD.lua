...
function onCreatePost()
  ...
  if not getPropertyFromClass('backend.ClientPrefs', 'data.hideHud') then addLuaText('foreverScore') end
  ...
  if not getPropertyFromClass('backend.ClientPrefs', 'data.hideHud') then addLuaText('foreverCornerMark') end
end

function onUpdate()
  if miniTimeTxt and getPropertyFromClass('backend.ClientPrefs', 'data.timeBarType') ~= 'Disabled' and curStep > 0 then
    setTextString('foreverCenterMark', ' - ' .. songName .. ' ' .. string.upper.. ' - ' .. milliToHuman(math.floor(getPropertyFromClass('backend.Conductor', 'songPosition') - noteOffset)) .. ' / ' .. milliToHuman(math.floor(songLength)) .. ' -')
  end
end
