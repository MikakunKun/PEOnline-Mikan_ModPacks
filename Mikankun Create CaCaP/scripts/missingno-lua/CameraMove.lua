...
function onUpdate(elapsed)
    ...
    if ForceCamPos then
        ...
        if FirstTime then
            FirstTime=false
            setProperty('camFollowPos.x',CameraFollow[1]+OffsetCam[1]+CamDisplaceX)
            setProperty('camFollowPos.y',CameraFollow[2]+OffsetCam[2]+CamDisplaceY)
        end
...
function goodNoteHit(membersIndex, noteData, noteType, isSustainNote)
    if ((mustHitSection or (songPath=='amusia')) and CamMove) or (FirstPerson) then
    CamDisplaceY=0
    if noteData == 1 then
...
function opponentNoteHit(membersIndex, noteData, noteType, isSustainNote)
    if (not mustHitSection or (songPath=='amusia')) and CamMove and not FirstPerson then
        CamDisplaceY=0
        if noteData == 1 then
